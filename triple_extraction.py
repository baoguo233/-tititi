...
class TripleExtractor:
    ...
    def ruler1(self, words, postags, roles_dict, role_index):
        ...
        """
        elif 'ADV' in role_info:
            print(4)
            o = ''.join([words[word_index] for word_index in range(role_info['ADV'][1], role_info['ADV'][2] + 1) if
                         postags[word_index][0] not in ['w', 'u', 'x']])
            if role_index > role_info['ADV'][1]:
                return '1', str(o + v)  # [v,o]
            else:
                return '1', str(v + o)  # [v,o]
        """
        return '4', []
...
def getevent(sentence):
    extractor = TripleExtractor()
    svos = extractor.triples_main(sentence)
    return svos
# '''测试'''
def test():
    content1 = "基本上是靠自力更生，也包括当时的苏联援助，采用了计划经济体制，克服了原始积累的障碍"
    ...
    for file in files:
        ...
        if file == 'label2.csv':
            ...
            for index, i in article_causality_sentence.iterrows():
                data = dict()
                if i['标签'] != '因为' and i['标签'] != '之所以-是因为':
                    event1 = extractor1.triples_main(i['原因'])
                    event2 = extractor1.triples_main(i['结果'])
            ...
            for data in datas:
                ...
                biglist.append([cause, tag, effect, sentence])
            list2 = []  # 去重
            for i in biglist:
                if i not in list2:
                    list2.append(i)
            ...
            article_anaysis.to_csv('E:\\Causal_events\\分类事件\\label2.csv', encoding='utf-8-sig')
        elif file == 'label1.csv':
            datas = list()
            for index, i in article_causality_sentence.iterrows():
                data = dict()
                if '因' not in i['标签'] and '所以' not in i['标签']:
                    event1 = extractor1.triples_main(i['原因'])
                    event2 = extractor1.triples_main(i['结果'])
                    if event1 and event2:
                        cause = event1[0]
            ...
            name = ['原因', '标签', '结果', '原句']
            article_anaysis = pd.DataFrame(columns=name, data=list2)
            article_anaysis.to_csv('E:\\Causal_events\\分类事件\\label1.csv', encoding='utf-8-sig')
        elif file == 'Other.csv':
            datas = list()
            for index, i in article_causality_sentence.iterrows():
                data = dict()
                tag = i['标签']
                sentence = i['原句']
                event = extractor1.triples_main(sentence)
                if len(event) > 1:
                    cause = event[0]
                    effect = event[1]
                    data['cause'] = cause
            ...
            name = ['原因', '标签', '结果', '原句']
            article_anaysis = pd.DataFrame(columns=name, data=list2)
            article_anaysis.to_csv('E:\\Causal_events\\分类事件\\Other.csv', encoding='utf-8-sig')